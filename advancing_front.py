...
class edge_storage:
    ...
    def get_edge(self):
        ...
        # self.q.rotate(random.randint(1, 100))
...
def is_point_in_polygon(x: node, edges: set[edge]) -> bool:
    px, py = x.get_coordinates()
    inside = False

    for e in edges:
        x1, y1 = e.n1.get_coordinates()
        x2, y2 = e.n2.get_coordinates()
...
class advancing_front:
    ...
    def __init__(
        self, edges: list[edge], tolerance=0.2, iterations=1000, is_animated=False
    ):

        self.initial_boundary = set(edges)
    ...
    def is_in_bounds(self, p) -> bool:
        return is_point_in_polygon(p, self.initial_boundary)
    ...
    def check_new_edge_validity(self, curr_edge, candidate_node):
        if not self.is_in_bounds(candidate_node):
            return False
        ...
        for _ in range(self.iterations):
            ...
            if not self.expand_mesh_with_node(curr_edge, candidate_node):
                ...
                if not self.expand_mesh_with_node(curr_edge, nearest_neighbor):
                    print(
                        f"Both Node and Neighbor failed, {curr_edge}, {candidate_node}, {nearest_neighbor}"
                    )
